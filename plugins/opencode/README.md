...
The Kilo Code CLI is an OpenCode fork with a compatible plugin API and config
schema, so `headroom wrap kilocode` reuses this same provider config and plugin
bundle against Kilo's own config directory (`~/.config/kilo/kilo.json`, or
`$KILO_CONFIG`).
